...
class LRUCache(object):
    ...
    def put(self, key, value):
        ...
        if key in self._data:
            # change the node into head
            node = self._data[key]
            self._moveNodeToHead(node)
            # change node value
            node.value = value
            # print('_moveNodeToHead')
            
            # if self._rootNode:
            #     print('_rootNode key', self._rootNode.key)
            # if self._tailNode:
            #     print('_tailNode key', self._tailNode.key)
            
        else:
            if self._nodeCount >= self._capacity:
                # evict last node
                # print('evict last')
                tmpNode = self._popLastNode()
                # delete hash
                # print('evict', tmpNode.key)
                self._data.pop(tmpNode.key)
            else:
                self._nodeCount += 1
                # print('not evict')
            # new node
            ...
            node.next = self._rootNode
            if self._rootNode is not None:
                self._rootNode.prev = node
            self._rootNode = node
    ...
    def _moveNodeToHead(self, node):
        # self.check()
        if self._rootNode == node:
            return
        preNode = node.prev
        if preNode is not None:
            if self._tailNode == node:
                self._tailNode = preNode
    ...
    def check(self):
        if self._rootNode and self._rootNode.prev is not None:
            raise Exception('_rootNode is not the head', self._rootNode.prev.key)
        if self._tailNode and self._tailNode.next is not None:
            raise Exception('tail is not tail', self._tailNode.next.key)
